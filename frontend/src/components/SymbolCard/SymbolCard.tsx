...
const SymbolCard = ({ id, onClick, price, activeSymbol }: SymbolCardProps) => {
  ...
  const containerClassNames = useMemo(() => `symbolCard ${isNotActive && "symbolCard--notActive"} ${isActive && "symbolCard--active"} ${isPriceGoingUp && "symbolCard--green"} ${isPriceGoingDown && "symbolCard--red"} ${isPriceSignificantlyDifferent && "symbolCard__shake"}`,
      [isActive, isNotActive, isPriceGoingUp, isPriceGoingDown, isPriceSignificantlyDifferent]);
